...
export const NotificationsHeader = () => {
  ...
  return (
    <Div className="mb-1 px-1 mt-1 sticky top-15 flex flex-col md:flex-row justify-between bg-gray-100 dark:bg-black w-full rounded-2xl">
      ...
      <Div className="flex flex-wrap gap-3 h-fit">
        {notificationFilters.map((_filter) => (
          <Div key={_filter.title} className="px-4 py-2 rounded-full bg-indigo-50 dark:bg-gray-800 font-medium shadow cursor-pointer hover:bg-indigo-200">
            {_filter.title}
          </Div>
        ))}
      </Div>
